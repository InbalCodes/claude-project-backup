...
function Start-RestoreMode {
    ...
    if ($wantResume -and $selectedRemote.Count -gt 0) {
        ...
        [void]$sb.AppendLine('per project, whenever you want to view or continue an old conversation')
        [void]$sb.AppendLine('in the desktop app:')
        [void]$sb.AppendLine('')
        ...
        [void]$sb.AppendLine('IMPORTANT - this is not a one-time fix. Tested 5 ways (foreground open')
        [void]$sb.AppendLine('vs closed, background session running vs "claude stop"-ed, then')
        [void]$sb.AppendLine('re-attached): sidebar visibility always tracks a LIVE CLI process, with')
        [void]$sb.AppendLine('no exception found. Close the terminal and the session disconnects;')
        [void]$sb.AppendLine('"claude stop" a background one and it disappears from the sidebar')
        [void]$sb.AppendLine('entirely. Either way, running "claude --resume" or "claude attach <id>"')
        [void]$sb.AppendLine('again always brings it right back - your transcript data is never at')
        [void]$sb.AppendLine('risk, only the sidebar convenience is temporary. Treat step 1-4 above as')
        [void]$sb.AppendLine('something you repeat each time, not a setup you do once.')
        [void]$sb.AppendLine('')
